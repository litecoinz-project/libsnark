...
std::vector<int64_t> find_wnaf(const uint64_t window_size, const bigint<n> &scalar)
{
    const uint64_t length = scalar.max_bits(); // upper bound
...
T fixed_window_wnaf_exp(const uint64_t window_size, const T &base, const bigint<n> &scalar)
{
    std::vector<int64_t> naf = find_wnaf(window_size, scalar);
    std::vector<T> table(UINT64_C(1)<<(window_size-1));
    T tmp = base;
    T dbl = base.dbl();
    for (uint64_t i = 0; i < UINT64_C(1)<<(window_size-1); ++i)
    {
        table[i] = tmp;
        tmp = tmp + dbl;
    }
...
template<typename T, mp_size_t n>
T opt_window_wnaf_exp(const T &base, const bigint<n> &scalar, const uint64_t scalar_bits)
{
    uint64_t best = 0;
